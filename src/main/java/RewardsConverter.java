...
public class RewardsConverter {
    public static void main(String[] args) {
        ...
        System.out.println("Please enter airlines miles to convert to cash value: ");
        var input_value2 = scanner.nextLine();
        double miles;
        isMiles = true;

        try {
            miles = Double.parseDouble(input_value2);
        } catch (NumberFormatException exception) {
            System.out.println("Could not parse input value as a double, exiting");
            return;
        }
        System.out.println("converting " + input_value2 + " miles to cash");
        var rewardsValue2 = new RewardValue(miles, isMiles);
